...
def download_pdfs_from_url(url, visited_urls=None, session=None):
    ...
    # Look for pagination links and navigate them recursively
    ...
    # Recursively follow non-pagination internal links (e.g., other related content links)
    for link in soup.find_all('a', href=True):
        href = link['href']
        full_url = urljoin(url, href)

        # Only follow internal links (that are not PDFs or pagination)
